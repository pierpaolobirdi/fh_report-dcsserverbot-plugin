...
def post_process(players: dict, cfg: dict, server_name: str) -> dict:
    ...
    for ucid, hook_cfg in hook_players.items():
        ...
        # ── use points_order from server config to decide base for positioning ──
        points_order  = str(cfg.get("points_order") or "T").strip()
        base_credits  = float(player_data.get("session_points", real_credits)) if points_order in ("S", "B-S") else real_credits
        ...
        if credits_fixed is not None and credits_fixed is not False and credits_fixed != "":
            ...
            if credits_mode == "real":
                ...
                new_credits = _apply_credits_mode(credits_mode, base_credits, players, ucid)
